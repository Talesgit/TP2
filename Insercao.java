...
class Insercao extends Jogador{
	// ordenar pelo nasc(caso empate pelo nome) e imprimir
	public static void orderPrint(int[] ids, int k){
		List<Insercao> jogs = new ArrayList<>();
		// ler e add na lista jog de cada id
		for(int i = 0; i < k; i++){
			Insercao obj = new Insercao();
			obj.ler(ids[i]);
			jogs.add(obj);
		}
		...
		for(int i = 1; i < jogs.size(); i++){
			Insercao temp = jogs.get(i);
			int j = i-1;
		        while(j >= 0 && ( temp.get_born().compareTo(jogs.get(j).get_born()) < 0) ){
				jogs.set(j+1, jogs.get(j));
				j--;
			} // fim while
		        jogs.set(j+1, temp);	
		}
